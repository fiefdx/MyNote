...
class NoteHandler(BaseHandler):
    ...
    def get(self):
        ...
        if option == "rebuild_index":
            flag = Servers.IX_SERVER["NOTE"].index_delete_note_by_user(user)
            if flag:
...
def create_note(note_dict, user, handler, user_locale, user_key = ""):
    ...
    try:
        ...
        if note.type != 'Search' and note.type != 'All':
            ...
            if flag == True:
                # fetch the generated id, then index this note directly
                # (deterministic) rather than via the shared time-window batch.
                note = Servers.DB_SERVER["NOTE"].get_note_by_sha1(note.sha1, user)
                flag = Servers.IX_SERVER["NOTE"].index_note_by_id(note.id, user, key = user_key, db_note = Servers.DB_SERVER["NOTE"])
                if flag == True:
...
class DeleteHandler(BaseHandler):
    ...
    def get(self):
        ...
        user_info = Servers.DB_SERVER["USER"].get_user_from_db(user)
        try:
            flag = Servers.IX_SERVER["NOTE"].index_delete_note_by_user(user, merge = True)
            yield gen.moment
            if flag:
...
class ImportAjaxHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            LOG.debug("import notes %s by user[%s] flag: %s, rate: %s/%s, finish: %s, predict_total: %s", fname, user, flag["flag"], flag["tasks"], flag["total"], flag["finish"], flag["predict_total"])
            result = flag
...
def async_index(fname, user_info, key = "", index_batch_size = 1000, merge = False):
    ...
    multi_process_note_tea = MultiProcessNoteTea(CONFIG["PROCESS_NUM"])
    flag = Servers.IX_SERVER["NOTE"].index_delete_note_by_user(user_info.user_name)
    if flag:
