...
@summary:  change for category.json
@author: YangHaitao

Modified on 2014-06-09
@summary:  change for add note description
@author: YangHaitao
''' 
...
def process_mission( mission_queue, result_queue, db_file_path, exts = None, storage_path = None, key = "", old_key = "",  user = ""):
    ...
    logger.config_logging(file_name = ("import_note_pid_" + str( os.getpid() ) + '_' + date_time + '.log'), 
                ...
                if mission_one != 'mission complete':
                    ...
                    for i in mission_one[1]:
                        if ".json" not in i:
                            ...
                            total_count_file += 1
                            note = note_xml.get_note_from_xml(xml_path = fpath)
                            if old_key != "":
                            ...
                            if flag == True:
                                ...
                                if storage_path != None:
                                    target = os.path.join(storage_path, note_dict["type"], i)
